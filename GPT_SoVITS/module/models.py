...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, ssl, y, y_lengths, text, text_lengths):
        ...
        x, m_p, logs_p, y_mask, _, _ = self.enc_p(quantized, y_lengths, text, text_lengths, ge)
    ...
    @torch.no_grad()
    def decode(self, codes, text, refer, noise_scale=0.5, speed=1, result_length:int=None, overlap_frames:torch.Tensor=None, padding_length:int=None):
        def get_ge(refer):
            ge = None
        ...
        x, m_p, logs_p, y_mask, y_, y_mask_ = self.enc_p(quantized, y_lengths, text, text_lengths, ge, speed, result_length=result_length, overlap_frames=overlap_frames, padding_length=padding_length)
        z_p = m_p + torch.randn_like(m_p) * torch.exp(logs_p) * noise_scale
